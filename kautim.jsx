...
import {
  ...
  UserPlus, AtSign, CircleDollarSign, ClipboardCheck
} from 'lucide-react';
...
async function loadData() {
  ...
  } catch (e) { /* key not found yet, that's fine */ }
  return emptyData();
}
...
// Core per-bill math: subtotal -> tax -> service -> owed -> minus payments -> remaining
...
// Aggregate net balances across every bill, grouped by debtor -> payer
function computeLedger(data) {
  const pairs = {}; // key `${debtorId}__${payerId}` -> { debtorId, payerId, amount, billIds: [] }
  ...
  return (
    <span
      ...
      style={{
        ...
        padding: '2px 9px', borderRadius: 999, border: '1.5px dashed',
...
function EmptyState({ icon: Icon, title, body, action }) {
  return (
    <div style={{
      textAlign: 'center', padding: '48px 20px', border: '1.5px dashed var(--line)',
      borderRadius: 14, background: 'var(--paper-dim)',
    }}>
...
function Field({ label, children, hint }) {
  return (
    <label style={{ display: 'block' }}>
      <span style={{
        display: 'block', fontSize: 11.5, fontWeight: 600, color: 'var(--ink-soft)',
...
function TextInput(props) {
  return <input {...props} style={{ ...inputStyle, ...(props.style || {}) }} className={`ki-input ${props.className || ''}`} />;
}
function NumberInput(props) {
  return <input type="number" {...props} style={{ ...monoInputStyle, ...(props.style || {}) }} className={`ki-input ${props.className || ''}`} />;
}

function PrimaryButton({ children, icon: Icon, ...rest }) {
  return (
    <button {...rest} className="ki-btn-primary" style={{
      ...
      cursor: rest.disabled ? 'not-allowed' : 'pointer', opacity: rest.disabled ? 0.4 : 1,
      ...(rest.style || {}),
    }}>
      {Icon && <Icon size={16} />}
      {children}
...
function GhostButton({ children, icon: Icon, ...rest }) {
  return (
    <button {...rest} className="ki-btn-ghost" style={{
      display: 'inline-flex', alignItems: 'center', gap: 6, padding: '8px 13px',
      borderRadius: 9, border: '1.5px solid var(--line)', background: 'transparent',
      color: 'var(--ink)', fontFamily: "'Inter', sans-serif", fontWeight: 600, fontSize: 13.5,
      cursor: rest.disabled ? 'not-allowed' : 'pointer', opacity: rest.disabled ? 0.4 : 1,
      ...(rest.style || {}),
    }}>
      {Icon && <Icon size={14} />}
...
function IconButton({ icon: Icon, label, danger, ...rest }) {
  return (
    <button {...rest} title={label} aria-label={label} style={{
      display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
      width: 30, height: 30, borderRadius: 8, border: '1.5px solid var(--line)',
      background: '#fff', color: danger ? 'var(--owe)' : 'var(--ink-soft)', cursor: 'pointer',
      ...(rest.style || {}),
    }}>
      <Icon size={15} />
    </button>
  ...
  return (
    <div style={{ marginBottom: 22 }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, marginBottom: 18 }}>
        ...
        <button onClick={onRefresh} title="Pull latest from the group" aria-label="Refresh"
          style={{
            display: 'flex', alignItems: 'center', justifyContent: 'center', width: 34, height: 34,
            borderRadius: 9, border: '1.5px solid var(--line)', background: '#fff', color: 'var(--ink-soft)',
      ...
      <nav style={{ display: 'flex', gap: 4, borderBottom: '1.5px solid var(--line)', overflowX: 'auto' }}>
        {TABS.map(t => {
          ...
          return (
            <button key={t.id} onClick={() => setView(t.id)} className="ki-tab" style={{
              display: 'flex', alignItems: 'center', gap: 7, padding: '9px 13px 11px',
              background: 'transparent', border: 'none', cursor: 'pointer', whiteSpace: 'nowrap',
              borderBottom: active ? '2.5px solid var(--stamp)' : '2.5px solid transparent',
              marginBottom: -1.5,
            }}>
              ...
              <span style={{
                fontFamily: "'IBM Plex Mono', monospace", fontSize: 10, color: 'var(--ink-soft)',
                opacity: 0.6,
              }}>{t.code}</span>
            </button>
          );
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr auto', gap: 9, alignItems: 'end' }}>
          <Field label="Name">
            <TextInput value={name} onChange={e => setName(e.target.value)}
              onKeyDown={e => e.key === 'Enter' && addPerson()} placeholder="e.g. Darnish" />
          </Field>
          <Field label="Discord ID (optional)" hint="Lets the message actually @ping them">
            <TextInput value={discordId} onChange={e => setDiscordId(e.target.value.replace(/[^0-9]/g, ''))}
              onKeyDown={e => e.key === 'Enter' && addPerson()} placeholder="17–19 digit user ID" />
          </Field>
      ...
      <div>
        ...
        {data.roster.length === 0 ? (
          <EmptyState icon={Users} title="Nobody added yet"
            body="Add everyone in the group above. Once they're on the roster, you can put them on bills and tag them in messages." />
        ) : (
                ...
                {editingId === p.id ? (
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 9, minWidth: 0 }}>
                      <div style={{
                        width: 30, height: 30, borderRadius: 999, background: 'var(--paper-dim)',
                        border: '1.5px solid var(--line)', display: 'flex', alignItems: 'center', justifyContent: 'center',
                        fontFamily: "'Space Grotesk', sans-serif", fontWeight: 700, fontSize: 12.5, color: 'var(--ink-soft)',
                        flexShrink: 0,
                    ...
                    <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                      ...
                      <IconButton icon={Trash2} label="Remove" danger
                        onClick={() => removePerson(p.id)}
                        disabled={referencedIds.has(p.id)}
                        style={referencedIds.has(p.id) ? { opacity: 0.3, cursor: 'not-allowed' } : {}} />
                    </div>
                  </div>
                )}
                {referencedIds.has(p.id) && editingId !== p.id && (
                  <div style={{ fontSize: 11, color: 'var(--ink-soft)', marginTop: 6 }}>
                    On at least one bill, so they can't be removed — rename instead if needed.
                  </div>
                )}
              </div>
            ))}
...
function NewBillView({ data, mutate, myId, goToLedger }) {
  ...
  if (data.roster.length === 0) {
    return (
      <EmptyState icon={Receipt} title="Add people first"
        body="Head to the People tab and add everyone who was at dinner before you start splitting a bill." />
    );
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
      ...
      <div className="ki-card receipt-card">
        <div className="ki-card-title">Items</div>
              ...
              <div key={it.id} className="receipt-line" style={{ paddingTop: idx === 0 ? 0 : 14 }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 90px 64px 30px', gap: 8, alignItems: 'end', marginBottom: 9 }}>
                  ...
                  <Field label="Price (RM)">
                    <NumberInput value={it.unitPrice} min="0" step="0.01"
                      onChange={e => updateItem(it.id, { unitPrice: e.target.value })} placeholder="0.00" />
                  </Field>
                  <Field label="Qty">
                    <NumberInput value={it.quantity} min="1" step="1"
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: it.assignments.length ? 8 : 0 }}>
                  {data.roster.map(p => {
                    ...
                    return (
                      <button key={p.id} onClick={() => toggleParticipant(it.id, p.id)} className="ki-chip" style={{
                        padding: '5px 11px', borderRadius: 999, fontSize: 12.5, fontWeight: 600, cursor: 'pointer',
                        border: on ? '1.5px solid var(--ink)' : '1.5px solid var(--line)',
                        background: on ? 'var(--ink)' : '#fff', color: on ? 'var(--paper)' : 'var(--ink-soft)',
                      }}>
                        {p.name}
                      </button>
                    );
                    ...
                    {it.assignments.map(a => (
                      <div key={a.personId} className="leader-row" style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
                        ...
                        <input type="number" step="0.01" value={a.amount}
                          aria-label={`${personName(data.roster, a.personId)}'s share of ${it.name || 'this item'}`}
                          onChange={e => setAssignmentAmount(it.id, a.personId, e.target.value)}
                          style={{
                            width: 78, padding: '4px 7px', borderRadius: 7, border: '1.5px solid var(--line)',
                            fontFamily: "'IBM Plex Mono', monospace", fontSize: 12.5, textAlign: 'right', background: '#fff', color: 'var(--ink)',
                          }} />
                      </div>
                    ))}
                    {Math.abs(diff) > 0.02 && (
        ...
        <button onClick={addItem} className="ki-add-item" style={{
          marginTop: 14, display: 'flex', alignItems: 'center', gap: 6, padding: '9px 13px',
          borderRadius: 9, border: '1.5px dashed var(--line)', background: 'transparent',
          color: 'var(--ink-soft)', fontWeight: 600, fontSize: 13.5, cursor: 'pointer', width: '100%', justifyContent: 'center',
        }}>
      ...
      <div className="ki-card">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
          ...
          <Field label="Receipt total (optional)" hint="Catches odd rounding">
            <NumberInput value={draft.receiptTotal} min="0" step="0.01" placeholder="0.00"
              onChange={e => setDraft(d => ({ ...d, receiptTotal: e.target.value }))} />
          </Field>
        </div>
      </div>
      ...
      {open && (
        <div style={{ marginTop: 16 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 9, marginBottom: 14 }}>
            {Object.entries(summary.perPerson).map(([pid, p]) => (
              <div key={pid} style={{
                ...
                padding: '8px 11px', borderRadius: 9, background: 'var(--paper-dim)', fontSize: 13,
              }}>
                <div>
                ...
                {pid !== bill.payerId && (
                  <span style={{
                    ...
                    color: round2(p.remaining) <= 0.004 ? 'var(--settled)' : 'var(--owe)',
                  }}>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            <GhostButton icon={MessageSquare} onClick={() => onMessageThis(bill.id)}>Draft a message for this</GhostButton>
            <GhostButton icon={Trash2} onClick={() => onDelete(bill.id)} style={{ color: 'var(--owe)', borderColor: 'var(--owe-bg)' }}>Delete bill</GhostButton>
          </div>
        </div>
      )}
    </div>
...
function LedgerView({ data, mutate, roster, goToMessage }) {
  ...
  function addPayment(billId, payment) {
    mutate({
      ...data,
      bills: data.bills.map(b => b.id === billId ? { ...b, payments: [...(b.payments || []), payment] } : b),
    });
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 22 }}>
      <div>
        ...
        {ledger.length === 0 ? (
          <EmptyState icon={Wallet} title="All settled up"
            body="No outstanding balances right now. Once you save a bill, anything owed will show up here." />
        ) : (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 10 }}>
            {ledger.map(p => (
              <div key={`${p.debtorId}__${p.payerId}`} className="ki-card" style={{ padding: '13px 15px' }}>
                <div style={{ fontSize: 13, color: 'var(--ink-soft)' }}>
                  <strong style={{ color: 'var(--ink)' }}>{personName(roster, p.debtorId)}</strong> owes <strong style={{ color: 'var(--ink)' }}>{personName(roster, p.payerId)}</strong>
                </div>
                <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontWeight: 700, fontSize: 19, color: 'var(--owe)', marginTop: 5 }}>
                  {fmt(p.amount)}
                </div>
              </div>
            ))}
      ...
      <div>
        ...
        {data.bills.length === 0 ? (
          <EmptyState icon={Receipt} title="No bills yet"
            body="Split your first one in the New Bill tab — it'll show up here once it's saved." />
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {[...data.bills].sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0)).map(bill => (
              <BillRow key={bill.id} bill={bill} roster={roster}
                onAddPayment={addPayment} onDelete={deleteBill} onMessageThis={goToMessage} />
            ))}
          </div>
        )}
      </div>
...
const TONE_COPY = {
  polite: {
    label: 'Polite',
    plea: "If you get a chance, please settle this up whenever's convenient — thank you!",
  },
  default: {
    label: 'Default',
    plea: "Please settle up when you get a chance — let's not let this drag on.",
  },
  savage: {
    label: 'Savage',
    plea: 'Failure to settle within the deadline below will result in **public reminders** until this is resolved.',
  },
};
...
function MessagePreview({ text }) {
  // tiny, safe renderer for just **bold** and -# subtext lines — not a full markdown engine
  ...
  return (
    <div style={{
      background: 'var(--discord-bg)', borderRadius: 12, padding: '16px 18px',
      fontFamily: "'IBM Plex Mono', monospace", fontSize: 13, lineHeight: 1.7, color: '#DBDEE1',
...
function MessageView({ data, roster, presetBillId }) {
  ...
  useEffect(() => {
    if (presetBillId) { setScopeType('bill'); setBillId(presetBillId); }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [presetBillId]);
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div className="ki-card">
          ...
          <div style={{ display: 'flex', gap: 6, marginTop: 2 }}>
            {Object.entries(TONE_COPY).map(([key, t]) => (
              <button key={key} onClick={() => setTone(key)} style={{
                ...
                border: tone === key ? '1.5px solid var(--ink)' : '1.5px solid var(--line)',
                background: tone === key ? 'var(--ink)' : '#fff', color: tone === key ? 'var(--paper)' : 'var(--ink-soft)',
              }}>{t.label}</button>
            ))}
          </div>
        </Field>
        ...
        <Field label="Extra notes (optional)" hint="Your own jokes, threats, or context go here — not auto-generated">
          <textarea value={extraNotes} onChange={e => setExtraNotes(e.target.value)} rows={3}
            placeholder="e.g. specific inside jokes about who keeps forgetting to pay…"
      ...
      <div>
        ...
        <div style={{ fontSize: 11.5, color: 'var(--ink-soft)', marginTop: 8, lineHeight: 1.5 }}>
          Pings as a real Discord mention for anyone with a saved Discord ID — everyone else shows as plain @name text.
        </div>
        <div style={{ marginTop: 12 }}>
          <PrimaryButton onClick={copyMessage} icon={copied ? Check : Copy}>
  ...
  return (
    <div className="ki-card">
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 10 }}>
        <button onClick={() => setScopeType('bill')} disabled={billsWithBalance.length === 0} style={{
          padding: '7px 13px', borderRadius: 9, fontSize: 13, fontWeight: 600,
          cursor: billsWithBalance.length === 0 ? 'not-allowed' : 'pointer',
          border: scopeType === 'bill' ? '1.5px solid var(--ink)' : '1.5px solid var(--line)',
          background: scopeType === 'bill' ? 'var(--ink)' : '#fff', color: scopeType === 'bill' ? 'var(--paper)' : 'var(--ink-soft)',
          opacity: billsWithBalance.length === 0 ? 0.4 : 1,
        }}>One bill</button>
        <button onClick={() => setScopeType('payer')} disabled={payerOptions.length === 0} style={{
          padding: '7px 13px', borderRadius: 9, fontSize: 13, fontWeight: 600,
          cursor: payerOptions.length === 0 ? 'not-allowed' : 'pointer',
          border: scopeType === 'payer' ? '1.5px solid var(--ink)' : '1.5px solid var(--line)',
          background: scopeType === 'payer' ? 'var(--ink)' : '#fff', color: scopeType === 'payer' ? 'var(--paper)' : 'var(--ink-soft)',
          opacity: payerOptions.length === 0 ? 0.4 : 1,
        }}>Everything owed to someone</button>
      </div>
      {scopeType === 'bill' ? (
  ...
  return (
    <div style={{ background: 'var(--paper)', minHeight: '100%', padding: '20px 16px 60px' }}>
      ...
      <div style={{ maxWidth: 720, margin: '0 auto' }}>
        ...
        {!storageOk && (
          <div style={{
            padding: '10px 14px', borderRadius: 10, background: 'var(--owe-bg)', color: 'var(--owe)',
